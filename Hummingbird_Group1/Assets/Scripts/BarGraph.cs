...
public class BarGraph : MonoBehaviour
{
    ...
    /*
     * Call this function to create the graph shown in the main menu
     */
    ...
    /*
     * This function calculates all the x and y positions on the canvas to draw the graph components
     * GetAxisLabel will create the labels along the x and y axes and add any suffixes (such as "s" for seconds)
     */
    private void ShowGraph(List<int> avg, List<int> values, Func<int, string> getAxisLabelX = null, Func<float, string> getAxisLabelY = null)
    {
        ...
        // Important graph container values - height, width
        // and the width of the bars, and the highest value
        ...
        // Create the bars and average line segments
        for (int i = 0; i < values.Count; i++)
        {
            // The x and y position of the top of the bars
            // on the canvas wrt graphcontainer
            ...
            // Create average line segment from this point to the next one
            if (i < values.Count - 1)
            {
                float avgPosX = xPos;
            ...
            // Create the x axis label for this bar
            RectTransform labelXDuplicated = Instantiate(labelX);
            labelXDuplicated.SetParent(graphContaner, false);
            labelXDuplicated.gameObject.SetActive(true);
            labelXDuplicated.anchoredPosition = new Vector2(xPos, -labelXDuplicated.sizeDelta.y * 0.5f);
            labelXDuplicated.GetComponent<TextMeshProUGUI>().text = getAxisLabelX(i);
        }

        // Create the y axis labels and also draw gridlines from them
        int separatorCount = 10;
        for (int i = 0; i <= separatorCount; i++)
        {
            RectTransform labelYDuplicated = Instantiate(labelY);
            labelYDuplicated.SetParent(graphContaner, false);
            labelYDuplicated.gameObject.SetActive(true);
            float normalizedY = i * 1f / separatorCount;
            labelYDuplicated.anchoredPosition = new Vector2(labelYDuplicated.sizeDelta.x * -0.5f, normalizedY * graphHeight);
            labelYDuplicated.GetComponent<TextMeshProUGUI>().text = getAxisLabelY(normalizedY * yMax);
